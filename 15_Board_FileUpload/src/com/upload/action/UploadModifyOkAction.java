...
public class UploadModifyOkAction implements Action{
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		int upload_no = Integer.parseInt(multi.getParameter("upload_no")); // 히든으로 넘어온 값
		...
		if(res > 0) {			
			// If there's something you need to take, then use setforward~!
			// if you don't have anything, then use redirect method instead.
			session.setAttribute("errMsg", "<script> alert('수정 성공~!'); </script>");
			forward.setRedirect(true); // 가져가는 값 없기 때문~! think about it!
			forward.setPath("upload_content.do?no="+upload_no);
		}else if(res == -1) {
			out.println("<script> alert('비번 틀림'); history.back(); </script>");			
		}else {
